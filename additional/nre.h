...
enum {	CFLAGS = REG_EXTENDED | REG_ICASE | REG_NOSUB | REG_NEWLINE,
	...
	GFLAGS = REG_NULL | REG_ANCH | REG_LITERAL | REG_AUGMENTED | REG_WHICH,
...
struct Rex {
	...
	uchar prog;	// progress mark
	short serial;	// subpattern number
	Rex *next;	// following part of reg exp
	Rex(int type=TEMP) : type(type), next(0), prog(0) { }
	virtual ~Rex();
	virtual Stat stat(Cenv*);
	virtual int serialize(int n);
	virtual int parse(uchar*, int, Rex*, Eenv*);
	VIRTUAL void print();
	int follow(uchar *s, int, Rex *cont, Eenv *env);
protected:
...
struct Rep : Dup {
	...
	Stat stat(Cenv*);
	int parse(uchar *,  int, Rex*, Eenv*);
	int dorep(int, uchar *, int, Rex*, Eenv*);
	void print();
};
